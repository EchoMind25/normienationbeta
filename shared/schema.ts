...
export const nfts = pgTable(
  ...
  (table) => [index("idx_nfts_owner").on(table.ownerId), index("idx_nfts_status").on(table.status)]
);
...
export const chatMessages = pgTable(
  ...
  (table) => [index("idx_chat_messages_room").on(table.roomId)]
);
...
export const galleryComments = pgTable(
  ...
  (table) => [index("idx_gallery_comments_item").on(table.galleryItemId)]
);
...
export const NORMIE_TOKEN = {
  ...
  twitter: "@NormieCEO",
  description:
    "A 4chan-inspired memecoin focused on everyday 'normie' culture. Relentless burns, community raids, and merch empire.",
};
